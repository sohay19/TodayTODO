...
extension Utils {
    ...
    static func transFirstDate(_ date:Date) -> Date? {
        return Calendar.current.date(from: Calendar.current.dateComponents([.month, .year], from: date))
    }
    static func transFirstDate(_ date:String) -> Date? {
        guard let curDate = dateStringToDate(date) else {
            return nil
        }
        return Calendar.current.date(from: Calendar.current.dateComponents([.month, .year], from: curDate))
    }
    //마지막 날로 변환
